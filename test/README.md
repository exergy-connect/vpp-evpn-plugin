...
#   not a 169.254 on the L3 BVI and not the anycast VIP on the VLAN BVI.
#   For traceroute across PEs with source validation, expose that loopback
#   through LCP in the matching Linux tenant VRF and advertise its /32 as
#   Type-5 through the control plane. See README.md, Anycast gateway vs L3
#   unnumbered vs unique tenant /32. The smoke wiring alone does not
#   configure BGP export.
#
