...
#   docker build -t vpp-evpn-plugin --build-arg VPP_VERSION=25.06-release .
#
# Extract the .so:
#   docker create --name evpn-tmp vpp-evpn-plugin
#   docker cp evpn-tmp:/usr/lib/x86_64-linux-gnu/vpp_plugins/evpn_plugin.so .
#   docker rm evpn-tmp
#
# Or run VPP with the plugin already installed:
#   docker run --rm -it --privileged vpp-evpn-plugin
